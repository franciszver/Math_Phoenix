 ...
 * Handles OCR pipeline (OpenRouter Vision)
 */
...
import { trackOCRMetrics, trackPipelineMetrics } from './metricsService.js';
...
/**
 * OCR pipeline: extract text from image using Vision API
 * @param {Buffer} imageBuffer - Image buffer
...
export async function extractTextFromImage(imageBuffer) {
  ...
  logger.info('Attempting Vision extraction...');
  ...
  try {
    ...
    // Graceful degradation: don't let a Vision failure throw out of the pipeline
    logger.error('Vision extraction failed, returning failure result');
    const totalLatency = Date.now() - pipelineStartTime;
    const finalResult = {
      text: '',
      confidence: 0,
